...
const soil2InchModel = (
  ...
  for (let i = 1; i <= constants.M; i++) {
    if (i === 1) {
      ...
      CP[i] =
        (((2400000 * buckets.top.bulkDensity) / 2.65 +
          4180000 * wvTopPerInch +
          (1 - buckets.top.bulkDensity / 2.65 - wvTopPerInch) * 1200) *
          (z[i + 1] - z[i - 1])) /
        (2 * constants.DT);
      K[i] =
        thermalConductivity(
      ...
      CP[i] =
        (((2400000 * buckets.bottom.bulkDensity) / 2.65 +
          4180000 * wvBottomPerInch +
          (1 - buckets.bottom.bulkDensity / 2.65 - wvBottomPerInch) * 1200) *
          (z[i + 1] - z[i - 1])) /
        (2 * constants.DT);
      K[i] =
        thermalConductivity(
...
const calcSoilTemps = (
  ...
  const { inches, z } = createConstants(
  ...
  if (rawEtData !== null) {
    ...
    if (DA > 0) {
      const currentDateIdx = tempPrcpData.findIndex(
        (arr) => arr[1] === -999 || arr[2] === -999 || arr[3] === -999
      );
